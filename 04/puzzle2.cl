...
(loop
   ...
   (let* (
      (next-boards (remove-if 'board-has-won *boards*))
      (removed (- (length *boards*) (length next-boards))))
         (format t "~D ~F removed!~%" removed (if (= 1 removed) "board was" "boards were"))
         (if (= 0 (length next-boards))
            (return))
         (setf *boards* next-boards)))

(format t "This is the last board left, making it tonight's biggest loser!~%")
(defconstant worst-board (car *boards*))
(print-board worst-board)

(defun board-flatten (board)
   (apply 'append (mapcar (lambda (vec) (coerce vec 'list)) board)))

(defconstant sum-of-unmarked
   (apply '+ (remove-if 'is-marked (board-flatten worst-board))))
